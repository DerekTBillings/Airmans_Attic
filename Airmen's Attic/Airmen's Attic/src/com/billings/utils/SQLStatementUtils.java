...
public class SQLStatementUtils {
	...
	private static Connection getConnection() {
		...
		try {
			conn = ConnManager.getNewConnection();
		} catch (Exception e) {
	...
	private static List getDataList(ResultSet rs, Class resultType) {
		...
		try {
			...
			} else if (Utils.isPrimitiveObject(resultType)) {
				dataList = getPrimitiveDataList(rs, resultType);
			} else {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private static List getPrimitiveDataList(ResultSet rs, Class resultType) throws Exception {
		List dataList = new ArrayList();
		
		while(rs.next()) {
			Object o = Utils.cast(rs.getObject(1), resultType);
			dataList.add(o);
		}
	...
	private static List getObjectDataList(ResultSet rs, String[] columnNames, Class resultType) throws Exception {
		...
		List<Method> methodNames = Utils.findSetterMethodsForAttributes(resultType, columnNames);
		...
		while(rs.next()) {
			...
			for (int i=0; i<columnCount; i++) {
				...
				Method currentMethod = methodNames.get(i);
				...
				currentMethod.invoke(result, value);
			}
